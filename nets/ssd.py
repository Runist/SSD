...
def SSD300(input_shape, num_classes=21):
    ...
    net['f38_norm_conf'] = layers.Conv2D(num_anchors * num_classes,
                                         kernel_size=(3, 3),
                                         padding='same',
                                         name='f38_norm_conf')(net['f38_norm'])
    net['f38_norm_conf_flat'] = layers.Flatten(name='f38_norm_conf_flat')(net['f38_norm_conf'])
    anchor = Anchor(img_size, min_size=30.0, max_size=60.0, aspect_ratios=[1, 2],
    ...
    net['f19_conf'] = layers.Conv2D(num_anchors * num_classes,
                                    kernel_size=(3, 3),
                                    padding='same',
                                    name='f19_conf')(net['f19'])
    net['f19_conf_flat'] = layers.Flatten(name='f19_conf_flat')(net['f19_conf'])

    anchor = Anchor(img_size, min_size=60.0, max_size=111.0, aspect_ratios=[1, 2, 3],
                    variances=[0.1, 0.1, 0.2, 0.2],
                    name='f19_anchor')
    ...
    net['f10_conf'] = layers.Conv2D(num_anchors * num_classes,
                                    kernel_size=(3, 3),
                                    padding='same',
                                    name='f10_conf')(net['f10'])
    net['f10_conf_flat'] = layers.Flatten(name='f10_conf_flat')(net['f10_conf'])

    anchor = Anchor(img_size, min_size=111.0, max_size=162.0, aspect_ratios=[1, 2, 3],
                    variances=[0.1, 0.1, 0.2, 0.2],
                    name='f10_anchor')
    ...
    net['f5_conf'] = layers.Conv2D(num_anchors * num_classes,
                                   kernel_size=(3, 3),
                                   padding='same',
                                   name='f5_conf')(net['f5'])
    net['f5_conf_flat'] = layers.Flatten(name='f5_conf_flat')(net['f5_conf'])

    anchor = Anchor(img_size, min_size=162.0, max_size=213.0, aspect_ratios=[1, 2, 3],
                    variances=[0.1, 0.1, 0.2, 0.2],
                    name='f5_anchor')
    ...
    net['f3_conf'] = layers.Conv2D(num_anchors * num_classes,
                                   kernel_size=(3, 3),
                                   padding='same',
                                   name='f3_conf')(net['f3'])
    net['f3_conf_flat'] = layers.Flatten(name='f3_conf_flat')(net['f3_conf'])

    anchor = Anchor(img_size, min_size=213.0, max_size=264.0, aspect_ratios=[1, 2],
                    variances=[0.1, 0.1, 0.2, 0.2],
                    name='f3_anchor')
    ...
    net['f1_conf'] = layers.Conv2D(num_anchors * num_classes,
                                   kernel_size=(3, 3),
                                   padding='same',
                                   name='f1_conf')(net['f1'])
    net['f1_conf_flat'] = layers.Flatten(name='f1_conf_flat')(net['f1_conf'])

    anchor = Anchor(img_size, min_size=264.0, max_size=315.0, aspect_ratios=[1, 2],
                    variances=[0.1, 0.1, 0.2, 0.2],
                    name='f1_anchor')
    ...
    # 8732,21
    net['conf'] = layers.Reshape((num_boxes, num_classes), name='conf_logits')(net['conf'])
...
if __name__ == '__main__':
    SSD300((300, 300, 3), num_classes=21)
